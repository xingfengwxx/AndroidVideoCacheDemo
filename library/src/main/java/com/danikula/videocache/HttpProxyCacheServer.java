...
public class HttpProxyCacheServer {
    ...
    public static final String TAG = "HttpProxyCacheServer";
    ...
    public void registerCacheListener(CacheListener cacheListener, String url) {
        ...
        synchronized (clientsLock) {
            try {
                Log.i(TAG, "CacheListener---1: " + url);
                //加密
                url = EncryptUtils.encryptDES2HexString(url.getBytes(), "ashd0303".getBytes(), "DES/ECB/PKCS5Padding", null);
                Log.i(TAG, "CacheListener---2: " + url);
                getClients(url).registerCacheListener(cacheListener);
            } catch (ProxyCacheException e) {
                LOG.warn("Error registering cache listener", e);
                Log.e(TAG, "Error registering cache listener");
            }
        }
    }
...
//        return String.format(Locale.US, "http://%s:%d/%s", PROXY_HOST, port, ProxyCacheUtils.encode(url));
        ...
        return String.format(Locale.US, "http://%s:%d/%s", PROXY_HOST, port, encryptUrl);
    }
